...
DatabaseManager::DatabaseManager()
{
    ...
    bool res = ss_db->initialize(iConfig->getParam(Config::SSD_IP).c_str(), iConfig->getParam(Config::SSD_PORT), iConfig->getParam(Config::SSD_USER).c_str(), iConfig->getParam(Config::SSD_PASSWORD).c_str(), iConfig->getParam(Config::SSD_NAME).c_str(), 1, 16384);
    if (!res)
    {
