...
bool MySQLDatabase::initialize ( const char* Hostname, unsigned int port, const char* Username, const char* Password, const char* DatabaseName, uint32
ConnectionCount, uint32 BufferSize )
{
    uint32 i;
    MYSQL* temp = NULL;
    MYSQL* temp2 = NULL;
...
bool MySQLQueryResult::nextRow()
{
	...
	MYSQL_FIELD *mfield = mysql_fetch_field_direct(mResult, i);
	mCurrentRow[i].setFieldName(mfield->name);
	mCurrentRow[i].setType(mfield->type);
    }
